...
const rememberFormData = ref(false)
const promoLoading = ref(false)
const promoResultTextVisible = ref(false)
const promoBtnVisible = ref(false)
const deliveryPoint = ref<selectedDeliveryPoint | null>(null)
const deliveryAddressInfo = ref<deliveryAddressInfoType | null>(null)
const needValidate = ref(false)
const promoResultText = ref('')
const deliveryItem = ref<deliveryListItemType | null>(null)
      ...
      <div :class="s.orderContent">
        ...
        <div :class="s.promoWrapper">
          <input-field
              ...
              @update:model-value="configuratorStore.updatePromoCodeValue"
          />
          <div :class="s.promoControls">
            ...
            <transition name="fade">
              <primary-btn
                  v-if="state.deliveryPromoCode && promoBtnVisible"
                  :active="true"
                  :class="s.promoUseBtn"
                  @click="checkPromo"
              >
            ...
            <transition name="fade">
              <p
                  ...
                  :class="[
                                    s.promoResultText,
                                    {
                                        [s.promoAccepted]: configuratorStore.promoAccepted,
                                        [s.promoDeclined]: !configuratorStore.promoAccepted
                                    }
                                ]"
              >
                {{ promoResultText }}
          ...
          <p :class="s.ordertotalPriceValue">
                        <span
                            :class="[
                                s.orderPriceDefault,
                                {
                                    [s.hasDiscount]: totalPriceWithDiscount !== totalPrice
                                }
                            ]"
                        >
                            {{ totalPrice }}
